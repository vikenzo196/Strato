...
const CSS = `
  ...
  *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
  ...
  .qsend{width:100%;display:flex;align-items:center;justify-content:center;gap:10px;padding:14px;border-radius:16px;border:1px solid var(--strokeSoft);background:linear-gradient(135deg,#bb8b72,#8c6856);color:#fff;font-weight:600;font-size:15px;cursor:pointer;box-shadow:inset 0 1px 0 rgba(255,255,255,.45), var(--elev2);transition:transform .12s}
...
/* ---- UX ecommerce/accessibilità ---- */
.trustrow{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:4px 18px 22px}
.trustitem{border:1px solid var(--strokeSoft);background:var(--glass2);border-radius:16px;padding:11px 8px;text-align:center;color:var(--text);font-size:11.5px;font-weight:700;line-height:1.25;box-shadow:inset 0 1px 0 var(--hi)}
.detailmeta{display:flex;gap:7px;flex-wrap:wrap;margin:10px 0 4px}
.metapill{display:inline-flex;align-items:center;border:1px solid var(--strokeSoft);background:var(--glass2);border-radius:999px;padding:6px 9px;font-size:12px;font-weight:700;color:var(--soft)}
.ddesc{font-size:14px;line-height:1.55;color:var(--text);margin:14px 2px 2px;white-space:pre-line}
.dswatches{pointer-events:auto}
.dswbox{min-width:58px;min-height:66px;border-radius:16px;padding:6px 5px;cursor:pointer}
.dswbox:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
.detailsheet .dbuy{position:sticky;bottom:0;z-index:5;background:linear-gradient(to top,var(--sheetbg) 72%,rgba(255,255,255,0));padding:14px 0 4px;margin-top:10px;border-top:1px solid var(--strokeSoft)}
.detailsheet .dadd2{max-width:none;background:linear-gradient(135deg,var(--accent),#8c6856);color:#fff}
.detailsheet .dadd2 svg{stroke:#fff}
.cartnote{margin:0 18px 14px;color:var(--soft);font-size:13px;line-height:1.45}
.crow .cactions{display:flex;flex-direction:column;align-items:center;gap:7px}
.cremove{border:0;background:transparent;color:var(--soft);font-family:inherit;font-size:11px;font-weight:700;cursor:pointer;padding:3px 5px;border-radius:8px}
.cremove:focus-visible,.qstep button:focus-visible,.dnav:focus-visible,.tb-btn:focus-visible,.qsend:focus-visible,.dadd2:focus-visible,.configbtn:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
.authsheet{text-align:center;background:var(--sheetbg);padding:22px 20px calc(24px + env(safe-area-inset-bottom))}
.authicon{width:52px;height:52px;margin:0 auto 10px;border-radius:18px;background:linear-gradient(135deg,var(--accent),#8c6856);display:grid;place-items:center;color:#fff;box-shadow:var(--elev2)}
.authicon svg{width:25px;height:25px;stroke:#fff}
.authsheet h4{justify-content:center;margin-bottom:8px}.authsheet p{margin:0 6px 16px;color:var(--soft);line-height:1.45}.authsheet .gbtn{margin-top:4px}
@media(min-width:560px){.detailsheet .dbuy{grid-column:1 / -1}.trustrow{grid-template-columns:repeat(3,1fr)}}
...
/* Sfondi fallback leggeri: gli sfondi reali caricati dall'admin restano in Supabase Storage. */
const WP_LIGHT = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1600 2400'%3E%3Cdefs%3E%3CradialGradient id='a' cx='25%25' cy='12%25' r='70%25'%3E%3Cstop stop-color='%23f0c7af'/%3E%3Cstop offset='1' stop-color='%23f7f2eb'/%3E%3C/radialGradient%3E%3CradialGradient id='b' cx='88%25' cy='28%25' r='60%25'%3E%3Cstop stop-color='%23cbd8bc' stop-opacity='.9'/%3E%3Cstop offset='1' stop-color='%23f7f2eb' stop-opacity='0'/%3E%3C/radialGradient%3E%3C/defs%3E%3Crect width='1600' height='2400' fill='url(%23a)'/%3E%3Crect width='1600' height='2400' fill='url(%23b)'/%3E%3C/svg%3E";
const WP_DARK = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1600 2400'%3E%3Cdefs%3E%3CradialGradient id='a' cx='25%25' cy='10%25' r='75%25'%3E%3Cstop stop-color='%236a4f3a'/%3E%3Cstop offset='1' stop-color='%23181411'/%3E%3C/radialGradient%3E%3CradialGradient id='b' cx='86%25' cy='30%25' r='65%25'%3E%3Cstop stop-color='%233f5140' stop-opacity='.75'/%3E%3Cstop offset='1' stop-color='%23181411' stop-opacity='0'/%3E%3C/radialGradient%3E%3C/defs%3E%3Crect width='1600' height='2400' fill='url(%23a)'/%3E%3Crect width='1600' height='2400' fill='url(%23b)'/%3E%3C/svg%3E";
...
function Card({ p, liked, onLike, onOpen, onEdit }) {
  ...
  return (
    <div className="card in" onClick={onOpen}>
      <div className="ph">
        <img src={colImg(c0)} alt={p.title} loading="lazy" />
        <button className="lk" onClick={(e) => { e.stopPropagation(); onLike(p.id); }} aria-label={liked ? "Rimuovi dai preferiti" : "Aggiungi ai preferiti"}>
...
export default function App() {
  ...
  const [authReason, setAuthReason] = useState("Accedi per salvare preferiti, inviare richieste ordine e seguire gli aggiornamenti.");
  ...
  const askLogin = (reason) => {
    setAuthReason(reason || "Accedi per salvare preferiti, inviare richieste ordine e seguire gli aggiornamenti.");
    setAuthOpen(true);
  };
  ...
  // Accessibilità: lo zoom del browser resta disponibile.

  // Scroll nativo: niente gesture globali invasive o transform su main.
  useEffect(() => {
    if (!mq) return;
  ...
  const toggleLike = async (id) => {
    if (!user) { askLogin("Accedi per salvare questo prodotto nei preferiti."); return; }
  ...
  const placeOrder = async () => {
    if (!user) { askLogin("Accedi per inviare la richiesta ordine e ricevere conferma su disponibilità e tempi."); return; }
  ...
  /* ---- navigazione ---- */
  const open = (t) => {
    if (!user && (t === "orders" || t === "profile" || t === "liked")) {
      askLogin(t === "orders" ? "Accedi per vedere e seguire i tuoi ordini." : "Accedi per gestire profilo e preferiti.");
      return;
    }
    ...
    <>
      ...
      <header className="topbar">
        ...
        <div className="tb-right">
          <button className="tb-btn bell" onClick={user ? openNotifs : () => askLogin("Accedi per vedere notifiche e aggiornamenti sugli ordini.")} aria-label="Notifiche">
            <Bell />{unread > 0 && <span className="belldot" />}
          </button>
          <button className="tb-btn right" onClick={() => open("profile")} aria-label={user ? "Profilo" : "Accedi"}>
            <img className="av" src={user ? (user.avatar || avatarURI(user.name)) : avatarURI("Ospite")} alt="" />
          </button>
        </div>
      </header>
      ...
      <div className="dockwrap">
        <div className="dock dock4">
          ...
          <button className={"dnav cart" + (cartOpen ? " act" : "")} onClick={() => setCartOpen(true)} aria-label="Carrello"><CartIcon />{cartCount > 0 && <span className="cartdot" />}</button>
      ...
      {cartOpen && (
        <CartSheet cart={cart} total={cartTotal} onClose={() => setCartOpen(false)} onStep={cartStep} onRemove={(i) => cartStep(i, -999)} onConfirm={placeOrder} />
      )}
      ...
      {authOpen && <AuthGate reason={authReason} onClose={() => setAuthOpen(false)} onGoogle={loginGoogle} />}
...
function Home({ prints, liked, onLike, onOpen, onEdit }) {
  const hero = useMemo(() => {
    ...
    return featured[0] || prints[0] || null;
  }, [prints]);
  return (
    <section className="screen on">
      <div className="px">
        <div className="kick">OGGETTI DI DESIGN STAMPATI IN 3D</div>
        <h1 className="hero">Design stampato in 3D, fatto su richiesta.</h1>
      </div>
      {hero && (
        <div className="herocard" key={hero.id} onClick={() => onOpen(hero.id)}>
          <img src={colImg(hero.cols[0])} alt={hero.title} loading="eager" />
      ...
      <div className="trustrow" aria-label="Punti di fiducia">
        <div className="trustitem">Produzione su richiesta</div>
        <div className="trustitem">Materiali selezionati</div>
        <div className="trustitem">Conferma prima del pagamento</div>
      </div>
...
function Detail({ p, prints, onClose, onOpen, onAdd, isAdmin, onSaveAddons, onEdit, onColorPhoto }) {
  ...
  const doAdd = (e) => {
    ...
    onAdd({
      ...
      pid: p.id, t: p.title, col: c?.name || "Unico", opt: optLabel, base: p.price, adds, price: unit,
      qty, img: c.img || "",
    });
  };
  ...
  return (
    <div className={"ipick on detailpop" + (closing ? " closing" : "")} role="dialog" aria-modal="true" aria-label={p.title} onClick={(e) => { if (e.target.classList.contains("ipick") || e.target.classList.contains("sheetwrap")) doClose(); }}>
        ...
        <div className="dgrid">
          <div className="dphoto">
            <div className="dgallery">
              {(c.imgs && c.imgs.length ? c.imgs : [colImg(c)]).map((src, gi) => (
                <img key={gi} className="dimg" src={src || colImg(c)} alt={p.title + " - " + (c?.name || "colore")} loading="lazy" />
              ))}
            </div>
          ...
          <div className="dopts">
            ...
            <div className="detailmeta">
              {p.categoryName && <span className="metapill">{p.categoryName}</span>}
              {p.material && <span className="metapill">{p.material}</span>}
              <span className="metapill">Fatto su richiesta</span>
            </div>
            {p.desc && <div className="ddesc">{p.desc}</div>}
            <div className="dlabel dcolor">Colore selezionato: {c?.name || "Unico"}</div>
            <div className="dswatches">
              {p.cols.map((cc, k) => (
                <button key={cc.id || k} type="button" className={"dswbox" + (k === ci ? " on" : "")} onClick={() => setCi(k)} aria-pressed={k === ci} aria-label={"Seleziona colore " + cc.name}>
          ...
          <div className="dbuy">
            ...
            <button className="dadd2" onClick={doAdd}><CartIcon /> Aggiungi al carrello · {eur(unit * qty)}</button>
          </div>
        </div>
        </div>
      </div>
...
/* ---- CARRELLO ---- */
function CartSheet({ cart, total, onClose, onStep, onRemove, onConfirm }) {
  const [closing, setClosing] = useState(false);
  const doClose = () => { if (closing) return; setClosing(true); setTimeout(onClose, 340); };
  useEffect(() => {
    const onKey = (e) => { if (e.key === "Escape") doClose(); };
    ...
    <div className={"ipick on" + (closing ? " closing" : "")} role="dialog" aria-modal="true" aria-label="Carrello" onClick={(e) => { if (e.target.classList.contains("ipick") || e.target.classList.contains("sheetwrap")) doClose(); }}>
      <div className="sheetwrap">
        ...
        <div className="sheet">
          ...
          <p className="cartnote">Non paghi ora. Invia una richiesta: ti confermeremo disponibilità, tempi e consegna prima del pagamento.</p>
          <div className="cartitems">
            {cart.length === 0 && <div className="cempty">Il carrello è vuoto.</div>}
            {cart.map((c, i) => (
              <div className="crow" key={c.key}>
                <img src={c.img || gimg(c.a || "#cfc4b4", c.b || "#9a8d79")} alt={c.t} loading="lazy" />
                <div className="cinfo">
                  <div className="cn">{c.t}</div>
                  <div className="cp">{c.col}{c.opt ? " · " + c.opt : ""}</div>
                  <div className="cprice">{eur(c.price)} cad. · subtotale {eur(c.price * c.qty)}</div>
                </div>
                <div className="cactions"><div className="qstep csmall"><button aria-label="Diminuisci quantità" onClick={() => onStep(i, -1)}>−</button><span>{c.qty}</span><button aria-label="Aumenta quantità" onClick={() => onStep(i, 1)}>+</button></div><button className="cremove" onClick={() => onRemove(i)}>Rimuovi</button></div>
              </div>
            ))}
          </div>
          <div className="cttot"><span>Totale</span><span>{eur(total)}</span></div>
          {cart.length > 0 && <button className="qsend" onClick={onConfirm}><Check /> Invia richiesta ordine</button>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className={"odone" + (closing ? " out" : "")} onClick={() => { setClosing(true); setTimeout(onDone, 380); }}>
      <div className="odcard">
        ...
        <div className="odt">Richiesta inviata</div>
        <div className="ods">È in attesa di conferma. Ti avviseremo appena disponibilità, tempi e consegna saranno verificati.</div>
      </div>
    </div>
  );
}
...
function InvoiceSheet({ o, isAdmin, onClose }) {
  ...
  return (
    <div className={"ipick on" + (closing ? " closing" : "")} role="dialog" aria-modal="true" aria-label="Riepilogo ordine" onClick={(e) => { if (e.target.classList.contains("ipick") || e.target.classList.contains("sheetwrap")) doClose(); }}>
      <div className="sheetwrap">
        <button className="sheetclose" onClick={doClose} aria-label="Chiudi"><ChevronDown /></button>
  ...
  return (
    <section className="screen on">
      ...
      <div className="pcard glass">
        <img className="pav" src={user.avatar || avatarURI(user.name)} alt={user.name} />
...
function AuthGate({ reason, onClose, onGoogle }) {
  useEffect(() => {
  ...
  return (
    <div className="ipick on authgate" role="dialog" aria-modal="true" aria-label="Accesso richiesto" onClick={(e) => { if (e.target.classList.contains("ipick") || e.target.classList.contains("sheetwrap")) onClose(); }}>
      <div className="sheetwrap">
        <button className="sheetclose" onClick={onClose} aria-label="Chiudi"><ChevronDown /></button>
        <div className="sheet authsheet">
          <div className="authicon"><User /></div>
          <h4>Accedi a Strato</h4>
          <p>{reason}</p>
