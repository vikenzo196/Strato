...
.wrap{max-width:680px;margin:0 auto;padding:12px 0 130px}
...
.thopt.on{background:var(--glassDock);box-shadow:inset 0 1px 0 var(--hi)}
.bgrow{display:flex;gap:10px;padding:0 18px;flex-wrap:wrap}
.bgsw{width:46px;height:46px;border-radius:14px;border:2px solid var(--strokeSoft);cursor:pointer;background:linear-gradient(135deg,#edc9b2,#bcd1ad)}
.bgsw[data-bg=b1]{background:linear-gradient(135deg,#edc9b2,#f0d9c2)}
.bgsw[data-bg=b2]{background:linear-gradient(135deg,#bcd1ad,#9ec39a)}
.bgsw[data-bg=b3]{background:linear-gradient(135deg,#eab9a6,#e0a890)}
.bgsw[data-bg=b4]{background:linear-gradient(135deg,#cdbce0,#b9a3d6)}
.bgsw.act{border-color:var(--text)}
...
/* ============================ CARD ==================================== */
function Card({ p, liked, onLike, onOpen }) {
  const c0 = p.cols[0];
  return (
...
export default function App() {
  ...
  if (!ready) {
    return (<><style>{CSS}</style><Raw html={GRADS_SVG} /><div className="boot">Strato…</div></>);
  }
  if (!user) {
    return (<><style>{CSS}</style><Raw html={GRADS_SVG} /><Login onGoogle={loginGoogle} /></>);
  }
  ...
  return (
    <>
      ...
      <main className="wrap">
        {tab === "home" && (
          <Home prints={prints} liked={liked} onLike={toggleLike} onOpen={openDetail} />
        )}
        {tab === "search" && (
          <Screen title={<><SearchI className="ticon" /> Cerca</>} icon={<SearchI />}>
            ...
            <Grid>
              {prints.filter((p) => matchQ(p, q)).map((p) => (
                <Card key={p.id} p={p} liked={liked(p.id)} onLike={toggleLike} onOpen={() => openDetail(p.id)} />
              ))}
            </Grid>
        ...
        {tab === "like" && (
          <Screen title="Piaciuti" icon={<span className="heartred"><HeartI /></span>} heart>
            {(() => {
              ...
              return <Grid>{list.map((p) => <Card key={p.id} p={p} liked onLike={toggleLike} onOpen={() => openDetail(p.id)} />)}</Grid>;
            })()}
          </Screen>
        )}
        {tab === "orders" && (
      ...
      {detail && (
        <Detail
          ...
          onAdd={addToCart} isAdmin={isAdmin}
      ...
      {inv && (
        <InvoiceSheet o={inv} onClose={() => setInvId(null)} />
      )}
...
function Home({ prints, liked, onLike, onOpen }) {
  const hero = prints[0];
  return (
    <section className="screen on">
      <div className="px">
      ...
      <Grid>
        {prints.map((p) => <Card key={p.id} p={p} liked={liked(p.id)} onLike={onLike} onOpen={() => onOpen(p.id)} />)}
      </Grid>
    </section>
  );
}
...
/* ---- DETTAGLIO ---- */
function Detail({ p, prints, onClose, onOpen, onAdd, isAdmin, onSaveAddons }) {
  ...
  return (
    <div className="detail on">
      <div className="dwrap">
        <button className="dback" onClick={onClose}><ChevronLeft /> Dettaglio</button>
        <div className="dgrid">
        ...
        {reco.length > 0 && (
          <div className="dreco">
            ...
            <Grid>{reco.map((rp) => <Card key={rp.id} p={rp} liked={false} onLike={() => { }} onOpen={() => onOpen(rp.id)} />)}</Grid>
          </div>
        )}
      </div>
    </div>
    ...
    <section className="screen on">
      ...
      {pend.map((o) => (
        <div className="banner glass" key={o.id}>
          <img className="bnav" src={o.avatar || avatarURI(o.who)} alt="" />
          <div className="txt" onClick={() => onOpen(o.id)} style={{ cursor: "pointer" }}>
            <b>Nuova richiesta</b> · {oTitle(o)} · {eur(o.total)} da {o.who}<br />
      ...
      {done.map((o) => (
        <div className="ord glass" key={o.id} onClick={() => onOpen(o.id)}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div className="s">{o.who} · {eur(o.total)}{o.date ? " · " + fmtDate(o.date) : ""}</div>
          </div>
...
/* ---- RIEPILOGO ORDINE ---- */
function InvoiceSheet({ o, onClose }) {
  return (
    <div className="ipick on" onClick={(e) => { if (e.target.classList.contains("ipick")) onClose(); }}>
      <div className="sheet inv">
        <button className="sheetclose" onClick={onClose}><ChevronDown /></button>
        <div className="invhead">
          <img className="invav" src={o.avatar || avatarURI(o.who)} alt="" />
          <div className="invwho">
            <div className="invtt">{o.who}</div>
...
function Profile({ user, theme, bg, onTheme, onBg, onLogout, isAdmin, onNewProduct }) {
  ...
  return (
    <section className="screen on">
      ...
      <div className="bgrow">
        {BGS.map((b) => (
          <button key={b || "def"} className={"bgsw" + (bg === b ? " act" : "")} data-bg={b || undefined} onClick={() => onBg(b)} />
        ))}
      </div>
